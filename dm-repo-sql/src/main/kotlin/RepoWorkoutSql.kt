...
import repository.DbWorkoutsResponse
...
//				errors = listOf(ModelForRequest.ApiError())
	...
	override suspend fun read(req: DbWorkoutIdRequest): DbWorkoutResponse {
		return safeTransaction({
			val result = (WorkoutTable innerJoin UsersTable).select { WorkoutTable.id.eq(req.id.asUUID()) }.single()
			val workoutModel: WorkoutModel = WorkoutTable.from(result)
			ExercisesTable.selectBatched { ExercisesTable.workout_id.eq(req.id.asUUID()) }.forEach {
				workoutModel.excersices.addAll(ExercisesTable.from(it))
			}
	...
	override suspend fun update(model: DbWorkoutModelRequest): DbWorkoutResponse {
		...
		return safeTransaction({
			...
			val result = WorkoutTable.select { WorkoutTable.id.eq(workoutModel.id.asUUID()) }.single()
	...
	override suspend fun search(req: DbWorkoutFilterRequest): DbWorkoutsResponse {
		return safeTransaction({
			val workoutModelList: MutableList<WorkoutModel> = (WorkoutTable innerJoin UsersTable)
				.selectBatched { WorkoutTable.id.eq(req.ownerId.asUUID()) }
				.map { WorkoutTable.from(it) }
				.single()

			workoutModelList.forEach {
				it.excersices = ExercisesTable.selectBatched { workout_id.eq(it.id.asUUID()) }
